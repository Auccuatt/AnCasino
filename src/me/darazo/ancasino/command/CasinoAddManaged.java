     ...
     if (this.args.length >= 2 && this.args.length <= 3) {
       ...
       if (!this.plugin.slotData.isSlot(this.args[1]).booleanValue())
       {
         ...
         if (this.args.length < 3) {
           ...
           if (this.plugin.permission.canCreate(this.player, typeName).booleanValue()) {
             this.type = typeName;
             this.owner = this.player.getName();
           } else {
             
             this.plugin.sendMessage(this.player, "Invalid type " + typeName);
             return Boolean.valueOf(true);
           }
         ...
         if (this.plugin.economy.has(this.player, createCost.doubleValue())) {
           this.plugin.economy.withdrawPlayer(this.player, createCost.doubleValue());
         } else {
         ...
         SlotMachine slot = new SlotMachine(this.name, this.type, this.owner, this.world, Boolean.valueOf(true));
