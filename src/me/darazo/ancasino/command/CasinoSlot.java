   ...
   extends AnCommand
 {
     ...
     if (this.args[2].equalsIgnoreCase("type") && this.args.length == 4) {
       ...
       if (this.plugin.typeData.isType(this.args[3]).booleanValue()) {
         ...
         if (this.plugin.permission.canCreate(this.player, this.type).booleanValue()) {
           ...
           if (this.plugin.economy.has(this.player, cost.doubleValue())) {
       ...
       if (!this.plugin.economy.has(this.player, amount.doubleValue())) {
         ...
         if (this.slot.getFunds().doubleValue() + amount.doubleValue() > this.plugin.typeData.getMaxPrize(this.slot.getType()).doubleValue()) {
           this.slot.setEnabled(Boolean.valueOf(true));
           sendMessage("Sufficient funds. Slot machine enabled.");
         } 
         
         this.slot.deposit(amount);
         this.plugin.economy.withdrawPlayer(this.player, amount.doubleValue());
         sendMessage("Deposited " + amount + " to " + this.slot.getName());
       }
     
     }
     else if (this.args[2].equalsIgnoreCase("withdraw") && this.args.length == 4) {
       ...
       this.plugin.economy.depositPlayer(this.player, amount.doubleValue());
   ...
   private void usage() {
     String[] messages = {
         ...
         "/casino slot <slot> withdraw <amount>" }; byte b; int i;
     String[] arrayOfString1;
     for (i = (arrayOfString1 = messages).length, b = 0; b < i; ) { String m = arrayOfString1[b];
       sendMessage(m);
       b++; }
