...
public class SlotData
{
	...
	{
	    Set<String> slots = this.plugin.configData.slots
	...
	String cXyz = String.valueOf(con.getX()) + "," + con.getY() + ","
		+ con.getZ();
	this.plugin.configData.slots.set(String.valueOf(path) + "name",
		slot.getName());
	this.plugin.configData.slots.set(String.valueOf(path) + "type",
		slot.getType());
	this.plugin.configData.slots.set(String.valueOf(path) + "owner",
		slot.getOwner());
	this.plugin.configData.slots.set(String.valueOf(path) + "world",
		slot.getWorld());
	this.plugin.configData.slots.set(String.valueOf(path) + "managed",
		slot.isManaged());
	this.plugin.configData.slots.set(String.valueOf(path) + "funds",
		slot.getFunds());
	this.plugin.configData.slots.set(String.valueOf(path) + "controller",
		cXyz);
	this.plugin.configData.slots.set(String.valueOf(path) + "location",
		xyz);
	this.plugin.configData.saveSlots();
    }
	...
	String type = this.plugin.configData.slots
		.getString(String.valueOf(path) + "type", "default");
	String owner = this.plugin.configData.slots
		.getString(String.valueOf(path) + "owner", null);
	String world = this.plugin.configData.slots
		.getString(String.valueOf(path) + "world", "world");
	Boolean managed = Boolean.valueOf(this.plugin.configData.slots
		.getBoolean(String.valueOf(path) + "managed", false));
	Double funds = Double.valueOf(this.plugin.configData.slots
		.getDouble(String.valueOf(path) + "funds", 0.0D));
    ...
    {
	String location = this.plugin.configData.slots
		.getString("slots." + name + ".controller");
	World world = Bukkit.getWorld(this.plugin.configData.slots
		.getString("slots." + name + ".world"));
