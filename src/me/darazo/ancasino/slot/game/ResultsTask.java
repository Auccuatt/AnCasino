   ...
   implements Runnable
 {
   ...
   public void run() {
     ...
     if (!results.isEmpty()) {
       ...
       for (Reward reward : results) {
         ...
         won = Double.valueOf(won.doubleValue() + reward.getMoney().doubleValue());
       } 
       ...
       {
         slot.withdraw(won);
         Double max = this.game.plugin.typeData.getMaxPrize(type.getName());
       ...
       this.game.plugin.sendMessage(player, (String)type.getMessages().get("noWin"));
     } 
 
     
     if (this.game.plugin.configData.trackStats.booleanValue()) {
       Stat stat; if (this.game.plugin.statsData.isStat(name).booleanValue()) {
   ...
   private ArrayList<Reward> getResults() {
     ...
     for (int i = 0; i < 5; i++) {
       ...
       else if (this.game.plugin.configData.allowDiagonals.booleanValue()) {
   ...
   private void playVictoryMusic() {
     ...
     this.game.scheduler.scheduleSyncDelayedTask((Plugin)this.game.plugin, new Runnable() {
           public void run() {
